...
addArg argSizes alloc@(sc, varMap, n, l, aasms) arg =
  let
    b = (argSizes !! n) == 8
...
addArgInline argSizes alloc@(sc, varMap, n, l, aasms) arg =
  let
    b = (argSizes !! n) == 8
...
genStmt fnMap (sc,m,i,l,aasm) (IRExpr e) = let
  ...
  in (sc,m,i',l',aasm ++ aasm')

genStmt fm (sc,m,i,l,aasm) (IRDecl s t scope) = let
  m' = Map.insert s i m -- assign ident s, temp number i
  in genStmt fm (sc,m',i+1,l,aasm) scope

genStmt fm (sc,m,i,l,aasm) (IRAsgn (IRIdent s size) _ e) = let
  temp = ATemp (m Map.! s) (getSize (IRIdent s size))
  (_,m',i',l',aasm') = genExp fm (sc,m,i,l,aasm) e temp
  in (sc,m',i',l',aasm')

genStmt fm (sc,m,i,l,aasm) (IRAsgn expr@(IRExpDereference (IRIdent s size) t _) _ e) = let
  temp = APtr (ATemp (m Map.! s) (getSize (IRIdent s size))) Nothing 0 0 (getSize expr)
  dest = ATemp i (getSize e)
  (_,_,i',l',aasm') = genExp fm (sc,m,i+1,l,aasm) e dest
  c = [AAsm [temp] Nop [ALoc dest]]
  in (sc,m,i',l',aasm' ++ c)

genStmt fm (sc,m,i,l,aasm) (IRAsgn expr@(IRExpDereference b t s) _ e) = let
  b' = ATemp i (getSize b)
  (_,m',i',l',aasm') = genExp fm (sc,m,i+1,l,aasm) b b'
  dest = ATemp i' (getSize e)
  (_,m'',i'',l'',aasm'') = genExp fm (sc,m',i'+1,l',aasm') e dest
  c = [AAsm [APtr b' Nothing 0 0 (getSize expr)] Nop [ALoc dest]]
  in (sc,m'',i'',l'',aasm'' ++ c)

genStmt fm (sc,m,i,l,aasm) (IRAsgn (IRExpFieldSelect base@(IRExpDereference _ _ _) _ _ size _) op e) = let
  dest = ATemp i (getSize e)
  (_,m',i',l',aasm') = genExp fm (sc,m,i+1,l,[]) e dest
  dest' = ATemp i' (getSize base)
  (_,m'',i'',l'',aasm'') = genExp fm (sc,m',i'+1,l',[]) base dest'
  ...
  in (sc,m'',i'',l'',aasm ++ aasm'' ++ aasm' ++ c)

genStmt fm (sc,m,i,l,aasm) (IRAsgn (IRExpFieldSelect base _ _ size _) op e) = let
  dest = ATemp i (getSize e)
  (_,m',i',l',aasm') = genExp fm (sc,m,i+1,l,[]) e dest
  dest' = ATemp i' (getSize base)
  (_,m'',i'',l'',aasm'') = genExp fm (sc,m',i'+1,l',[]) base dest'
  ...
  c = case ind of Just _ -> [AAsm [APtr (ATemp tNum True) ind (off) (additive + size) (getSize e)] Nop [ALoc dest]]
                  Nothing -> [AAsm [APtr (ATemp tNum True) ind (off + size) 0 (getSize e)] Nop [ALoc dest]]
  in (sc,m'',i'',l'',aasm ++ aasm' ++ (init aasm'') ++ c)

genStmt fm (sc,m,i,l,aasm) (IRAsgn (IRExpArraySubscript (IRIdent s _) index t size) op e) = let
  dest = ATemp i (getSize index)
  temp = APtr (ATemp (m' Map.! s) True) (Just (AIndex (getSize index))) 1 8 (getSize e)
  (_,m',i',l',aasm') = genExp fm (sc,m,i+1,l,aasm) index dest
  aasm'' = aasm' ++ genArrayCheck sc (ATemp (m' Map.! s) True) dest
  dest' = ATemp i' (getSize e)
  (_,m'',i'',l'',aasm''') = genExp fm (sc,m',i'+1,l',aasm'') e dest'
  ...
  in (sc,m'',i''+1,l'', aasm''' ++ b ++ d ++ c)

genStmt fm (sc,m,i,l,aasm) (IRAsgn (IRExpArraySubscript base index t size) op e) = let
  base' = ATemp i (getSize base)
  (_,m',i',l',aasm') = genExp fm (sc,m,i+1,l,aasm) base base'
  index' = ATemp i' (getSize index)
  (_,m'',i'',l'',aasm'') = genExp fm (sc,m',i'+1,l',aasm') index index'
  aasm''' = aasm'' ++  genArrayCheck sc base' index'
  val = ATemp i'' (getSize e)
  (_,m''',i''',l''',aasm'''') = genExp fm (sc,m'',i''+1,l'',aasm''') e val
  ...
  in (sc,m''',i''',l''', aasm'''' ++ b ++ c)

genStmt fm (sc,m,i,l,aasm) (IRBlock stmts) = let
  -- Keep scope alive, start new AAsm list, concat when finished.
  (_,m',i',l',aasm') = foldl (genStmt fm) (sc,m,i,l,[]) stmts
  in (sc,m',i',l',aasm ++ aasm')
...
genCtrl fm (sc,m,i,l,aasm) (Assert e _) = let
...
genCtrl fm (sc,m,i,l,aasm) (Return (Just expr) _) =
  let
    (_,_,i',l',aasm') = genExp fm (sc,m,i,l,[]) expr (AReg 0 (getSize expr))
  in
    (sc,m,i',l',aasm ++ aasm' ++ [ACtrl $ ARet (ALoc (AReg 0 (getSize expr)))])

genCtrl fm (sc,m,i,l,aasm) (Return Nothing _) =
  -- dummy value
  (sc,m,i,l,aasm ++ [ACtrl $ ARet (AImm 1)])
...
genExp _ (sc,varMap,n,l,aasm) (IRExpInt num _) dest =
  (sc,varMap,n,l, aasm ++ [AAsm [dest] Nop [AImm $ fromIntegral num]])
genExp _ (sc,varMap,n,l,aasm) (IRExpBool b) dest =
  (sc,varMap,n,l,aasm ++ [AAsm [dest] Nop [ABool b]])
genExp _ (sc,varMap,n,l,aasm) (IRIdent s size) dest =
  (sc,varMap,n,l,aasm ++ [AAsm [dest] Nop [ALoc $ ATemp (varMap Map.! s) (getSize $ IRIdent s size)]])
...
genExp f alloc@(sc,varMap,n,l,aasm) (IRExpTernary e1 e2 e3) dest = let
  (_,_,n1,e1l,e1Aasm) = genExp f (sc,varMap,n+1,l,[]) e1 (ATemp n False)
  (_,_,n2,e2l,e2Aasm) = genExp f (sc,varMap,n1,e1l,[]) e2 dest
  (_,_,n3,e3l,e3Aasm) = genExp f (sc,varMap,n2,e2l,[]) e3 dest
  e2Label = e3l
  e3Label = e3l + 1
  endLabel = e3l + 2
  ...
  outputAasm =
    ...
    ++ [ACtrl $ ALabel endLabel]
  in
    (sc,varMap,n3,e3l+3,aasm ++ outputAasm)

genExp f (sc,varMap,n,l,aasm) (IRExpUnOp op e) dest = let
  -- AAsm for operand
  (_,_,n1,l',aasm') = genExp f (sc,varMap, n + 1,l,aasm) e (ATemp n (getSize e))
  -- AAsm for the operation
  c  = [AAsm [dest] op [ALoc $ ATemp n (getSize e)]]
  in (sc,varMap, n1, l', aasm' ++ c)

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpFnCall fnName exprs _) dest =
...
genExp f (sc,varMap,n,l,aasm) (IRExpNull) dest =
  (sc,varMap,n,l, aasm ++ [AAsm [dest] Nop [AImm $ 0]])

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpAlloc t s) dest =
  genRealFn f alloc (IRExpFnCall "calloc" [IRExpInt (fromIntegral s) Dec,
                                           IRExpInt 1 Dec] 8) dest

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpAllocArray t expr s) dest = let
  (_,varMap',n',l',aasm') = genExp f (sc,varMap,n+1,l,aasm) expr $ ATemp n False
  locs = [ATemp n' False, ATemp n False]
  aasm'' = aasm' ++ genArrayAllocCheck sc (ATemp n False) s ++ [AAsm [ATemp (n' + 1) False] Add [ALoc $ ATemp n False, AImm $ fromIntegral (max (8 `div` (max s 4)) 1)], AAsm [ATemp n' False] Nop [AImm $ fromIntegral s], AAsm [AReg 12 False] Nop [ALoc $ ATemp n' False],
                     AAsm [AReg 13 False] Nop [ALoc $ ATemp (n' + 1) False],
                     AFnCall "calloc" dest locs []]
  in (sc,varMap',n'+2,l',aasm'' ++ [AAsm [APtr dest Nothing 0 0 False] Nop [ALoc $ ATemp n False]])

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpDereference (IRIdent s _) t _) dest =
  (sc,varMap,n,l,aasm ++ [AAsm [dest] Nop [ALoc $ APtr (ATemp (varMap Map.! s) True) Nothing 0 0 (getSize e)]])

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpDereference expr _ _) dest = let
  (_,varMap',n',l',aasm') = genExp f (sc,varMap,n+1,l,aasm) expr (ATemp n True)
  in (sc,varMap',n'+1,l',aasm' ++ [AAsm [dest] Nop [ALoc $ APtr (ATemp n True) Nothing 0 0 (getSize e)]])

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpFieldSelect base field t size _) dest = let
  (_,varMap',n',l',aasm') = genExp f (sc,varMap,n+1,l,aasm) base (ATemp n (getSize base))
  (tNum, ind, off, additive) = getPtrFromLastOp aasm'
  ...
  in (sc,varMap',n'+1,l',aasm' ++ c)

genExp f alloc@(sc,varMap,n,l,aasm) e@(IRExpArraySubscript base index t size) dest = let
  (_,varMap',n',l',aasm') = genExp f (sc,varMap,n+1,l,aasm) base (ATemp n True)
  (_,varMap'',n'',l'',aasm'') = genExp f (sc,varMap',n'+1,l',aasm') index (ATemp n' False)
  in (sc,varMap'',n'',l'',aasm'' ++  (genArrayCheck sc (ATemp n True) (ATemp n' False)) ++
...
genInlineFn f alloc@(sc,varMap, n, l, aasm) (IRExpFnCall fnName exprs _) dest ret =
  let
    allocs = scanl (genExpAcc f) alloc exprs
    lenMinusOne = (length allocs) - 1
    locs = map toLoc (zip (take lenMinusOne allocs) exprs)
    last@(_,varMap',n',l',aasm') = allocs !! lenMinusOne
...
genExpAcc f (sc,varMap,n,l,aasm) exp =
  genExp f (sc,varMap,n+1,l,aasm) exp (ATemp n (getSize exp))

genBinOp f (sc,varMap,n,l,aasm) (op,e1,e2) dest = let
  -- AAsm for left and right operand
  -- TODO: Make this more SSL friendly
  (_,_,n1,l',aasm') = genExp f (sc,varMap, n + 1,l, []) e1 (ATemp n (getSize e1))
  (_,_,n2,l'',aasm'') = genExp f (sc,varMap, n1 + 1,l', []) e2 (ATemp n1 (getSize e2))
  -- AAsm for the operation
  c  = [AAsm [dest] op [ALoc $ ATemp n (getSize e1), ALoc $ ATemp n1 (getSize e2)]]
  -- Questionable variable indexing here
  in (sc,varMap, n2, l'', aasm ++ aasm' ++ aasm'' ++ c)
