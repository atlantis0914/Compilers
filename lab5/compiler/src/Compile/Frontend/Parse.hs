...
type MState = (Map.Map String Bool, Map.Map String IdentType)
...
typedecl = do 
  ...
  (do pos' <- getPosition
      op <- asnOp
      e <- expr
      setState (f, Map.insert ident idType s)
      return $ PDecl ident idType pos (Just (PAsgn (PLId ident pos) op e False pos')))
   <|>
   (do setState (f, Map.insert ident idType s) 
...
complexLValue lval = 
  do whiteSpace
     (Text.Parsec.try  (do e <- brackets dec
                           pos <- getPosition
                           let e' = ExpInt e pos Dec
...
checkExprForFuckery :: Expr -> C0Parser ()
checkExprForFuckery (ExpBinMem PArrayRef e1 _ _) = do
  (_,idState) <- getState
...
probeArr m (Ident name _) cnt = 
  case (Map.lookup name m) of 
    Just typ -> numArrays typ cnt 0
    _ -> True
probeArr m _ _ = True

numArrays :: IdentType -> Int -> Int -> Bool
numArrays (IArray inner) match c = numArrays inner match (c+1)
numArrays IInt match c = (c == match)
numArrays _ _ _ = True 
...
arrayParser = do e' <- brackets expr
                 return $ \x -> ExpBinMem PArrayRef x e' dummyPos
